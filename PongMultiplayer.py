...
class Pelotagame:
    ...
    def rebotar(self):
        if self.x <= 0:
            self.reiniciar()
            self.puntuacion_J2 += 1
            
        if self.x + self.ancho >= VENTANA_HORI:
            self.reiniciar()
            self.puntuacion_J1 += 1
...
def main():
	...
	while jugando:
		...
		raqueta_1.moverJ1()
		raqueta_2.moverJ2()
		raqueta_1.golpearJ1(pelota)
		raqueta_2.golpearJ2(pelota)
		...
		#PUNTUACION
		ventana.blit(texto(str(pelota.puntuacion_J1), 100, WHITE), (VENTANA_HORI / 4, 10))
		ventana.blit(texto(str(pelota.puntuacion_J2), 100, WHITE), ((VENTANA_HORI * 3 / 4) - 20, 10))
		
		if pelota.puntuacion_J1==7:
			jugando = False
		elif pelota.puntuacion_J2==7:
			jugando = False
